...
mediante un array $A[1 \dots n]$, che soddisfa la proprietà di max-heap.

In particolare:

- la radice si trova in posizione $A[1]$;
- il padre del nodo in posizione $i > 1$ si trova in posizione $\lfloor i/2 \rfloor$;
...
$$ \lfloor n_b / 2 \rfloor + 1, \dots, n_b $$
