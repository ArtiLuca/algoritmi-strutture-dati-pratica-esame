...
la lunghezza della Longest Common Subsequence (LCS) `ell(i,j)` tra i prefissi
`X[1..i]` e `Y[1..j]` è definita come:

```math
\ell(i,j)=
\begin{cases}
0
& \text{se } i=0 \text{ oppure } j=0, \\[4pt]
\ell(i-1,j-1)+1
& \text{se } i,j>0 \text{ e } x_i=y_j, \\[4pt]
\max\{\ell(i-1,j),\ell(i,j-1)\}
...
- **Caratteri diversi:** se i caratteri correnti differiscono, cioè `x_i != y_j`, allora dobbiamo valutare se conviene scartare l'ultimo carattere di `X` oppure l'ultimo carattere di `Y`, prendendo il massimo tra le due possibilità.
