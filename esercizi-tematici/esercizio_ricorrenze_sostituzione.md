...
### (1) $T(n) = T(n-1) + n$ ammette soluzione $O(n^2)$
...
(1 - 2)n + 1 = -n + 1 \le 0
$$
Questa disuguaglianza è verificata per $n \ge 1$. Quindi possiamo concludere che, con $c = 1$ e $n_0 = 1$, vale $T(n) = O(n^2)$ per ogni $n \ge n_0$.

---

### (2) $T(n) = T(n/2) + \Theta(1)$ ammette soluzione $O(\log n)$

Per dimostrare che la ricorrenza ammette soluzione $O(\log n)$, dobbiamo dimostrare che $\exists c > 0, \exists n_0 \in \mathbb{N}$ per cui vale $T(n) \le c \log_2 n$.
...
Scegliendo, ad esempio, $d = 1$ e $c = 2$, con $n_0 = 2$ (per avere logaritmi ben definiti e positivi), la disuguaglianza è soddisfatta e dimostriamo che $T(n) = O(\log n)$.

---

### (3) $T(n) = 2T(n/2) + n$ ammette soluzione $\Theta(n \log n)$

Per dimostrare che la ricorrenza ammette soluzione $\Theta(n \log n)$, dimostriamo separatamente i limiti superiore ($O$) e inferiore ($\Omega$).
...
Scegliendo $c = 2$, abbiamo la disuguaglianza $2n \log_2 n - n \le 2n \log_2 n$, che è vera per qualsiasi scelta di $n_0 \ge 2$. Dunque $T(n) = O(n \log n)$.
...
Scegliendo $d = 1$, la disuguaglianza diventa $n \log_2 n \ge n \log_2 n$, che è sempre vera per $n > 0$. Dunque una scelta di $d = 1$ e $n_0 = 2$ è sufficiente per concludere che $T(n) = \Omega(n \log n)$.

Avendo dimostrato sia il limite superiore che inferiore, possiamo concludere che $T(n) = \Theta(n \log n)$.

---

### (4) $T(n) = 2T(n/2 + 17) + n$ ammette soluzione $O(n \log n)$
*TODO*
