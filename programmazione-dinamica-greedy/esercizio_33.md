...
In generale una soluzione per il problema è $S = D_0, D_1, \dots, D_n$, ovvero i distributori a cui decidiamo di fermarci per fare il pieno. 
Più specificamente, possiamo definire una soluzione per il problema come l’insieme dei distributori $S = D_{i_0} \dots D_{i_k}$ 
dove $D_{i_0} = A$ e $D_{i_k} = B$.
...
Ovvero tale che $\vert{}S^{\prime}\vert{} < \vert{}S^{\ast}\vert{}$, 
che è assurdo perché abbiamo definito $S^{\ast}$ come soluzione ottima per il problema con il minor numero di soste.
