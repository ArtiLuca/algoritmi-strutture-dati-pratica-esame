...
Per garantire che la complessità temporale rimanga $\Theta(n)$ serve cura nella gestione dei puntatori di ricerca della procedura ausiliaria: essi avanzano in modo sempre crescente, non venendo mai ripristinati all'indietro. Questo garantisce che ogni elemento dell'array venga analizzato per la ricerca di scambi al massimo una volta.
...
* **Complessità temporale:** È $\Theta(n)$. L'array viene inizialmente scansionato interamente una volta per il conteggio e durante le fasi di scambio, i puntatori di ricerca `ptr1` e `ptr2` avanzano sempre in avanti oppure restano invariati. Questo garantisce che ogni elemento dell'array venga ispezionato al massimo una volta.
* **Spazio:** Si usa uno spazio $\Theta(1)$ in quanto le uniche variabili usate sono quelle di conteggio.
* **Numero di confronti:** Nel caso peggiore sono $\le 5n$. Servono $2n$ confronti per il conteggio e al più $3n$ per gli avanzamenti incrociati di `ptr0`, `ptr1`, `ptr2`.
* **Numero di scambi:** Nel caso peggiore sono $\le n$. Ogni scambio posiziona definitivamente almeno un elemento, con il valore corretto spostato all'indice `ind`. Il caso peggiore si ha quando l'intero array è ordinato in senso opposto, per cui serviranno $n$ scambi per ordinarlo completamente.
...
Ad ogni passo viene esaminato l'elemento $A[\text{mid}]$. A seconda del valore (0, 1, o 2), l'elemento viene scambiato nella rispettiva regione e si aggiornano i puntatori, riducendo di 1 la dimensione della regione degli elementi sconosciuti ($\text{high} - \text{mid} + 1$). Il ciclo termina quando $\text{mid} > \text{high}$, nel qual caso la regione dei valori da analizzare risulta vuota e quindi l'array finale risulta ordinato in senso crescente.
