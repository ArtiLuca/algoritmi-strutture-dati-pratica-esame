...
Sia $A_{ij}^{\ast}$ una soluzione ottima per il problema; la scelta greedy seleziona come prima attività $a_m$, ovvero l’attività tale che $f_m = \min\{f_l : a_l \in A_{ij}^{\ast}\}$.
...
Dopodiché, l'algoritmo scorre tutte le attività $a_m$ con $m = 2 \dots n$, tenendo traccia dell’indice dell’ultima attività scelta (in questo caso l’attività $a_1$). Si aggiunge un’attività alla soluzione ottimale solo se questa inizia dopo (o nel momento esatto) che finisce l’ultima attività selezionata, ovvero $s_m \ge f_{\text{last}}$, e si aggiorna l’indice dell’ultima attività selezionata.
